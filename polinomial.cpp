...
{
	polinom P;
	printf("Ââåäèòå ñòåïåíü ïîëèíîìà\n");
	if (scanf("%d", &(P.k)) == EOF) { printf("îøèáêà â read_PP"); P.k = -1; return P; }
	P.mas = new rational_number[P.k + 3];
	if (P.mas == NULL) { printf(""); P.k = -1; return P; }
	printf("Ââåäèòå êîýôôèöèåíòû ïîëèíîìà. Íå çàáûâàéòå, ÷òî íîëü - òîæå êîýôôèöèåíò.\n");
	for (int i = P.k; i >= 0; i--)
		P.mas[i] = read_QQ();
	return P;
}
...
int free_P(polinom P)
{
	...
	{
		q = P.mas[i];
		if (free_Q(q) == -1) { printf("îøèáêà â free_P"); return -1; }
	}
	Q = P.mas;
	//check
	if (Q == NULL) { printf("ïóñòîé óêàçàòåëü â free_P"); return -1; }
	//
	delete[] Q;
	Q = NULL;
	return 0;
...
polinom FAC_P_Q(polinom P)//7
{
	...
	if (P.mas == NULL) { printf("ïóñòîé óêàçàòåëü â FAC_P_Q"); P.k = -1; return P; }//check
	Nok = new uint8_t[P.mas[0].m];
	Nod = new uint8_t[P.mas[0].n];
	if (Nod == NULL || Nok == NULL) { printf("ïóñòîé óêàçàòåëü â FAC_P_Q"); P.k = -1; return P; }//check
	NokM = P.mas[0].m;
	NodN = P.mas[0].n;
	...
	for (i = P.k; i > 0; i--)
	{
		...
		if (Zn == NULL || Ch == NULL) { printf("ïóñòîé óêàçàòåëü â FAC_P_Q"); P.k = -1; return P; }//check
	...
	for (i = P.k; i >= 0; i--)
	{
		...
		else { printf("ïóñòîé óêàçàòåëü â FAC_P_Q"); P.k = -1; return P; }
		P1.mas[i].n = x;
...
polinom MUL_PP_P(polinom P1, polinom P2)//8
{
	...
	P.mas = new rational_number[P.k + 3];
	//check
	if (P.mas == NULL) { printf("ïóñòîé óêàçàòåëü â MUL_P_P"); P.k = -1; return P; }
	//
	for (i = P.k; i >= 0; i--) 
	{ 
		P.mas[i].m = 1;
		...
		if (P.mas[i].ch == NULL || P.mas[i].zn == NULL) { printf("ïóñòîé óêàçàòåëü â MUL_P_P"); P.k = -1; return P; }//check
		P.mas[i].ch[0] = 0;
		P.mas[i].zn[0] = 1;
		P.mas[i].sign = 0;
	}
...
int polinomial()
{
	printf("ââåäèòå íîìåð äåéñòâèÿ ñîãëàñíî èíñòðóêöèè â äîêóìåíòàöèè\n1) Ñëîæåíèå ìíîãî÷ëåíîâ\n2) Âû÷èòàíèå ìíîãî÷ëåíîâ\n3) Óìíîæåíèå ìíîãî÷ëåíà íà ðàöèîíàëüíîå ÷èñëî\n4) Óìíîæåíèå ìíîãî÷ëåíà íà x^k\n5) Ñòàðøèé êîýôôèöèåíò ìíîãî÷ëåíà\n6) Ñòåïåíü ìíîãî÷ëåíà\n7)Âûíåñåíèå èç ìíîãî÷ëåíà ÍÎÊ çíàìåíàòåëåé êîýôôèöèåíòîâ è ÍÎÄ ÷èñëèòåëåé\n8)\n9)\n10)\n11)\n12)\n13\n");
	int x, k; char c;
